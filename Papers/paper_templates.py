...
class OrnamentalBorder(PaperTemplate):
    ...
    def _draw_looped_corner(self):
        ...
        # Draw page outline
        svg += f'  <rect x="{x}" y="{y}" width="{w}" height="{h}" '
        svg += f'fill="none" stroke="#000000" stroke-width="{1.2*self.thickness}"/>\n'
        ...
        svg += f'  <path d="'
        svg += f'M {x + 1 * step} {y + 3 * step} '
        svg += f'L {x + 3 * step} {y + 3 * step} '
        svg += f'L {x + 3 * step} {y + 1 * step} '
        svg += f'L {x + w - 3 * step} {y + 1 * step} '
        svg += f'L {x + w - 3 * step} {y + 3 * step} '
        svg += f'L {x + w - 1 * step} {y + 3 * step} '
        svg += f'L {x + w - 1 * step} {y + h - 3 * step} '
        svg += f'L {x + w - 3 * step} {y + h - 3 * step} '
        svg += f'L {x + w - 3 * step} {y + h - 1 * step} '
        svg += f'L {x + 3 * step} {y + h - 1 * step} '
        svg += f'L {x + 3 * step} {y + h - 3 * step} '
        svg += f'L {x + 1 * step} {y + h - 3 * step} '
        svg += f'L {x + 1 * step} {y + 3 * step} '
        svg += f'Z" '
        svg += f'fill="#FFFFFF" stroke="#000000" stroke-width="{self.thickness * 0.5}"/>\n'
        return svg
