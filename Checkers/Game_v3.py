...
class Board:
    ...
    def get_valid_moves(self, piece):
        """Return dict of (row,col) -> [list of captured pieces] for all valid moves."""
        moves = {}
        directions = []
        ...
        # ── Simple (non-capture) steps ──────────────────────────────────────
        for dr, dc in directions:
            nr, nc = piece.row + dr, piece.col + dc
            if 0 <= nr < ROWS and 0 <= nc < COLS and self.board[nr][nc] is None:
                moves[(nr, nc)] = []

        # ── Capture chains (recursive DFS) ─────────────────────────────────
        self._find_captures(piece.row, piece.col, piece.color, piece.king,
                            captured_so_far=[], moves=moves)

        # If any captures exist, only captures are legal (mandatory capture rule)
        captures = {k: v for k, v in moves.items() if v}
        return captures if captures else moves

    def _find_captures(self, row, col, color, king, captured_so_far, moves):
        """DFS over all jump chains; updates `moves` with every reachable landing."""
        directions = []
        if color == RED   or king: directions += [(-1, -1), (-1, +1)]
        if color == BLACK or king: directions += [(+1, -1), (+1, +1)]

        found_any = False
        for dr, dc in directions:
            mid_r, mid_c = row + dr,       col + dc
            land_r, land_c = row + dr*2,   col + dc*2
            if not (0 <= land_r < ROWS and 0 <= land_c < COLS):
                continue
            mid_piece = self.board[mid_r][mid_c]
            # Must jump over an enemy not already captured in this chain
            if mid_piece is None or mid_piece.color == color:
                continue
            if mid_piece in captured_so_far:
                continue
            if self.board[land_r][land_c] is not None:
                continue

            found_any = True
            new_captured = captured_so_far + [mid_piece]

            # A landing square reached with MORE captures beats a plain step
            if (land_r, land_c) not in moves or len(new_captured) > len(moves[(land_r, land_c)]):
                moves[(land_r, land_c)] = new_captured

            # Check if this piece would be kinged on landing (affects further jump dirs)
            becomes_king = king or (color == RED and land_r == 0) or (color == BLACK and land_r == ROWS - 1)

            # Recurse — temporarily mark mid square empty so we don't re-jump it
            saved = self.board[mid_r][mid_c]
            self.board[mid_r][mid_c] = None
            self._find_captures(land_r, land_c, color, becomes_king,
                                new_captured, moves)
            self.board[mid_r][mid_c] = saved
        ...
        for move, skip in valid.items():
            ...
            if skip:
                tmp.remove([tmp.get(s.row, s.col) for s in skip
                            if tmp.get(s.row, s.col) is not None])
            moves.append(tmp)
    return moves
...
# ── Game loop ─────────────────────────────────────────────────────────────────
def _any_captures(board, color):
    """Return True if any piece of `color` has at least one capture available."""
    for piece in board.get_all_pieces(color):
        caps = {}
        board._find_captures(piece.row, piece.col, piece.color, piece.king,
                             captured_so_far=[], moves=caps)
        if any(v for v in caps.values()):
            return True
    return False

def _legal_moves(board, piece, turn):
    """
    Return the legal moves for `piece` given board-wide mandatory-capture rule.
    If any friendly piece can capture, this piece may only capture (or nothing).
    """
    moves = board.get_valid_moves(piece)
    if _any_captures(board, turn):
                        ...
                        if selected:
                            if (row, col) in valid:
                                ...
                                if piece and piece.color == turn:
                                    selected = piece
                                    valid    = _legal_moves(board, piece, turn)
                        else:
                            if piece and piece.color == turn:
                                selected = piece
                                valid    = _legal_moves(board, piece, turn)
